local AdNode = import(".AdNode")
local FJAdNode = class("FJAdNode",AdNode)
--[[
	界面处理需要保证就算是错误数据也做到不崩溃
	在获取到数据的时候进行checkData操作
]]
--[[function FJAdNode:onCreate()
	-- body
	self:init("AdNode")
	self:schedulerFunc(function()
		...
		if cur_page == 0 then
			self.node.Left_flag:show()
			self.node.Right_flag:hide()
		elseif cur_page == self.page_game_num - 1 then
			self.node.Left_flag:hide()
			self.node.Right_flag:show()
		end
	end,AdNode.CHANGE_TIME,false)
end

function FJAdNode:initAdPageView(data)
...
function FJAdNode:pageViewEvent(tag,event)
	-- body
	local cur_page = self.node.Ad_PageView:getCurrentPageIndex()
...
function FJAdNode:getAdImgNode(idx)
