...
--激活模块
function WebViewPart:activate(orientation,url,keyword,inputParam)
    WebViewPart.super.activate(self,CURRENT_MODULE_NAME)
    
    if self.view then
        self:_initWebView(url,keyword,orientation,inputParam)
    end
...
function WebViewPart:sendStringToServer( inputParam )
    ...
    net_manager:sendMsg(buff_str,buff_lenth,MsgDef.MSG_POST_USER_INFO,SocketConfig.GAME_ID)  
end


-- webview 
-- 初始化webview
function WebViewPart:_initWebView(url,keyword,orientation,inputParam)

    printInfo("WebViewPart:_initWebView url =%s,keyword = %s,orientation = %s,inputParam = %s",url,keyword,orientation,inputParam)
    local lua_bridge = global:getModuleWithId(ModuleDef.BRIDGE_MOD)
    lua_bridge:changeActivityOrientation(orientation)
