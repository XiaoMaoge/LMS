...
function GameEndLayer:onCreate()
	-- body
	self:init("GameEndLayer")
	self.node.result_list:setItemModel(self.node.result_panel)
	--self.card_sprite = import(".CardFactory","app/part/mjcard/")
...
function GameEndLayer:getEndType(player)
	...
	if player.coin == 0 then --没输没赢
		return self.res_base .. "/room/resource/end/nothing.png"
	elseif bit._and(result,MahjongHuCode.ZI_MO) ~= 0 then --自摸
		return self.res_base .. "/room/resource/end/zimo.png"
	elseif bit._and(result,MahjongHuCode.DIAN_PAO) ~= 0 then 
		return self.res_base .. "/room/resource/end/dianpao.png"
	elseif bit._and(result,MahjongHuCode.WIN) ~= 0 then
		return self.res_base .. "/room/resource/end/hupai.png"
	elseif bit._and(result,MahjongHuCode.LIU_JU) ~= 0 then
		return self.res_base .. "/room/resource/end/liuju.png"
	else --输
		return self.res_base .. "/room/resource/end/lose.png"
	end
end
...
function GameEndLayer:setData(data , tablepos)
	-- body
