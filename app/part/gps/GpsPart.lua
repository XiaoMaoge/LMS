...
--激活模块
function GpsPart:activate(tablepos,flag)
...
function GpsPart:sendGpsMsg(tablepos,flag)
	...
	if flag > 1 then
		...
		net_mode:registerMsgListener(SocketConfig.MSG_GET_PLAYERS_GPS_INFO_ACK, handler(self,GpsPart.msgGetPlayersGpsInfoAck))
...
function GpsPart:msgGetPlayersGpsInfo()				--请求GPS
	...
	net_mode:sendProtoMsg(msg_get_players_gps_info,SocketConfig.MSG_GET_PLAYERS_GPS_INFO,SocketConfig.GAME_ID)
...
function GpsPart:msgGetPlayersGpsInfoAck(data,appID) 	--GPS回调
	...
	print("GpsPart msgGetPlayersGpsInfoAck: ", "dfffffffffffffffffff就是了看到飞机零件数量大幅减少了雕刻技法")
